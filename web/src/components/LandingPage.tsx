...
const copy = {
  zh: {
    ...
    heroLead: '先把表达练稳，',
    heroAccent: '再开始正式录制。',
    heroBody: 'RhythmCoach 把提纲、提词、录音与节奏反馈放进一次排练里，帮助演讲者和播客创作者在按下录制键之前发现停顿、失速与超时。',
    explore: '看看如何工作',
    ...
    feedbackKicker: '03 · 复盘',
    feedbackTitle: '每次只改一个最重要的问题。',
    feedbackBody: '训练结束后查看实际用时、发声占比、长停顿与完成度。反馈会解释为什么重要，并给出下一次可以立即执行的重点。',
    duration: '实际用时',
    voiceRatio: '发声占比',
    longPauses: '长停顿',
    completion: '完成度',
    ...
    finalBody: '我可以是：演讲者、播客er、YouTuber、Vlogger、英语学习者、粤语练习者……',
    backToTop: '返回顶部',
  ...
  en: {
    ...
    heroBody: 'RhythmCoach brings outlines, prompting, recording, and pacing feedback into one rehearsal, so speakers and podcasters can catch pauses, rushed passages, and timing problems before the real take.',
    ...
    feedbackKicker: '03 · Review',
    feedbackTitle: 'Improve one important thing at a time.',
    feedbackBody: 'Review actual duration, voice ratio, long pauses, and completion after each rehearsal. The feedback explains why it matters and gives you one concrete focus for the next run.',
    duration: 'Duration',
    ...
    finalBody: 'I can be a speaker, podcaster, YouTuber, vlogger, English learner, or Cantonese learner…',
      ...
      <main>
        <section className="landing-hero" aria-labelledby="landing-title">
          <div className="landing-hero-copy">
            ...
            <div className="landing-trust-line"><ShieldCheck size={16} /> {text.trust}</div>
          </div>

          <div className="landing-window landing-editor-window" aria-label={lang === 'zh' ? '脚本准备界面预览' : 'Script preparation preview'}>
            ...
            <div className="landing-editor-layout">
              <div className="landing-editor-main">
                <div className="landing-editor-heading">
                  <div><span>{text.podcastMode}</span><h2>{text.mockTitle}</h2></div>
                ...
                <div className="landing-script-block"><span>{text.opening}</span><p>{text.openingText}</p></div>
                <div className="landing-script-block landing-script-block-active"><span>{text.outline}</span><p>{text.outlineText}</p></div>
                <div className="landing-script-block"><span>{text.closing}</span><p>{text.closingText}</p></div>
              </div>
        ...
        <section className="landing-feature-section" id="landing-workflow" aria-labelledby="workflow-title">
          <div className="landing-section-copy">
            <span className="landing-section-kicker">{text.workflowKicker}</span>
          ...
          <div className="landing-window landing-prompter-window" aria-label={lang === 'zh' ? '提词排练界面预览' : 'Rehearsal prompting preview'}>
            <div className="landing-prompter-topbar">
              <div className="landing-recording-state"><span /><Mic2 size={15} /> {text.recording}</div>
              <div className="landing-prompter-time"><span>{text.elapsed}</span><strong>02:18</strong><span>{text.total} 05:00</span></div>
              <button type="button" aria-label={lang === 'zh' ? '暂停' : 'Pause'}><Pause size={16} fill="currentColor" /></button>
            </div>
            <div className="landing-prompter-body">
              <div className="landing-time-rail" aria-hidden="true"><span>02:00</span><i /><span>03:00</span><i /><span>04:00</span></div>
        ...
        <section className="landing-feature-section landing-feedback-section" id="landing-feedback" aria-labelledby="feedback-title">
          <div className="landing-window landing-feedback-window" aria-label={lang === 'zh' ? '节奏反馈界面预览' : 'Pacing feedback preview'}>
            <div className="landing-feedback-head">
              <div><span>{lang === 'zh' ? '本次排练' : 'Latest rehearsal'}</span><h3>{text.mockTitle}</h3></div>
              <BarChart3 size={22} />
            </div>
            ...
            <div className="landing-focus-card">
              ...
              <div><span>{text.focusLabel}</span><p>{text.focus}</p></div>
            </div>
          </div>

          <div className="landing-section-copy landing-feedback-copy">
            <span className="landing-section-kicker">{text.feedbackKicker}</span>
            <h2 id="feedback-title">{text.feedbackTitle}</h2>
            <p>{text.feedbackBody}</p>
            <button type="button" className="landing-primary-cta" onClick={onOpenApp}>{text.openApp} <ArrowRight size={18} /></button>
          </div>
        </section>
        ...
        <section className="landing-final-section" aria-labelledby="final-title">
          ...
          <p>{text.finalBody}</p>
          <button type="button" className="landing-primary-cta" onClick={onOpenApp}>{text.startRehearsal} <ArrowRight size={18} /></button>
        </section>
      </main>

      <footer className="landing-footer">
        <a className="landing-footer-brand" href="#/"><img src="./rhythmcoach.svg" alt="" width="30" height="30" /><strong>RhythmCoach</strong></a>
        <div>
